...
class BatchStorageService {
  ...
  Future<void> optimizeEcgStorage(String sessionId) async {
    ...
    try {
      ...
      while (true) {
        ...
        _updateStatus(sessionId, OptimizationPhase.processingEcgBatches, 
  ...
  Future<void> cleanupOldData({
    ...
    try {
      ...
      if (sessionRetention != null) {
        ...
        _updateStatus('cleanup', OptimizationPhase.cleanupCompleted,
  ...
  EcgSampleBatch _createEcgBatch(List<EcgSample> samples, int batchNumber) {
    ...
    if (samples.length > 1) {
      final totalDuration = samples.last.timestamp
  ...
  void _updateStatus(
    ...
    _statusController.add(OptimizationStatus(
      sessionId: sessionId,
      phase: phase,
      progress: progress,
      total: total,
...
class StorageOptimizationStats {
  ...
  int get totalSamples => totalEcgSamples + totalHeartRateSamples + totalHrvSamples;
...
final batchStorageServiceProvider = Provider<BatchStorageService>((ref) {
  ...
  return BatchStorageService(sampleRepository, sessionRepository);
});

/// Placeholder for session repository provider
final sessionRepositoryProvider = Provider<SessionRepository>((ref) {
  throw UnimplementedError('SessionRepository provider not implemented');
});